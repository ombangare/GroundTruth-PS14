...
4. Upload the **`packages/shapefile/DISTRICT_BOUNDARY.zip`** file provided in this repository.
...
4. From this API Settings screen, you need to copy exactly 3 important strings:
   - **Project URL**: Found under "Project URL" (looks like `https://xxxxxx.supabase.co`)
   - **anon / public key**: Found under "Project API Keys". Scroll down, it is tagged as `anon` and `public`. *(You will paste this into the Frontend `.env.local`!)*
   - **service_role / secret key**: Found right below the anon key. It is tagged as `service_role` and `secret`. *(You will paste this into the Backend `.env`!)*
5. Still in Project Settings, click on **Authentication** (or **Auth**) from the left menu, and then click **JWT Settings**. 
6. Scroll down to find your **JWT Secret** and copy it.

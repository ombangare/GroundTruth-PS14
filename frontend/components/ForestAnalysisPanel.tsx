        ...
        <div className="mt-4 flex flex-col gap-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="p-6 rounded-lg bg-[#050811] border border-space-line shadow-xl flex flex-col justify-center items-center">
              <h3 className="text-emerald-400 font-bold mb-6 font-mono text-sm uppercase tracking-widest border-b border-space-line/50 pb-2 w-full text-center">
                Temporal Forest Extent (km²)
              </h3>
              <div className="w-full h-64 mt-4">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={(data.raw_gee_data || data).time_series || [
                    { name: (data.raw_gee_data || data).start_year, area: (data.raw_gee_data || data).forest_start },
                    { name: (data.raw_gee_data || data).end_year, area: (data.raw_gee_data || data).forest_end }
                  ]}>
                    ...
                    <Tooltip 
                      ...
                      formatter={(value) => [`${value} km²`, 'Forest Area']}
                    />
                    <Area type="monotone" dataKey="area" stroke="#34d399" strokeWidth={3} fillOpacity={0.2} fill="#34d399" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              <div className="mt-8 grid grid-cols-2 gap-8 w-full text-center font-mono">
                  <div className="flex flex-col bg-space-line/10 p-3 rounded-lg border border-space-line/30">
                      ...
                      <span className="text-lg text-white font-bold">{(data.raw_gee_data || data).total_area} km²</span>
                  </div>
                  <div className="flex flex-col bg-space-line/10 p-3 rounded-lg border border-space-line/30">
